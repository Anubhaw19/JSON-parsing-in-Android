...
public final class QueryUtils {
     ...
     */
    //A Private Constructor ,so that no one can create any instance of this class.
    // all the public methods can be called without any object instance.
    private QueryUtils() {
    }
    ...
    // this is a function which is returning a ArrayList<Quake> ,without any input parameters.
    public static ArrayList<Quake> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<Quake> earth = new ArrayList<Quake>();
        ...
        try {
            ...
            JSONArray EarthARRAY=baseJSON.getJSONArray("features");
            ...
            for(int i=0;i<EarthARRAY.length();i++)
            {
                ...
                String loaction=properties.getString("place");
              long time     =properties.getLong("time");


                // DATE format changing:-
               // long TimeInMS=Long.parseLong(time); //TimeInMs=time in milliseconds. // converting String into Long.
               // Date dateObject=new Date(TimeInMS);
               // SimpleDateFormat dateFormat=new SimpleDateFormat("MM DD,yyyy");
               // String dateToDisplay=dateFormat.format(dateObject);
                //creating a new object of Quake;
                //using all the extracted data as a parameter.
                Quake quake=new Quake(magnitude,loaction,time);
                earth.add(quake);//adding this object [quake] to the [earth] ArrayList;
/*               another alternative way to add this object
        earth.add(new Quake(magnitude,loaction,time));
        */
